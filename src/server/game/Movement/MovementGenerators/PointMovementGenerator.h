...
public:
    PointMovementGenerator(uint32 _id, float _x, float _y, float _z, ForcedMovement forcedMovement, float _speed = 0.0f, float orientation = 0.0f, Movement::PointsArray const* _path = nullptr,
        ...
        bool reverseOrientation = false) // mod-playerbots
        ...
        _chargeTargetGUID(chargeTargetGUID), _forcedMovement(forcedMovement), _animTier(animTier), _reverseOrientation(reverseOrientation)
    {
        if (_path)
...
private:
    ...
    bool _reverseOrientation; // mod-playerbots
    bool _stalled{};
